...
public class DefaultGameProfileMarshal implements BinaryTagMarshal<GameProfile> {
    ...
    @Override
    public BinaryTag<?> serialize(BinaryTagDriver binaryTagDriver, GameProfile gameProfile) {
    ...
    public GameProfile deserialize(BinaryTagDriver binaryTagDriver, BinaryTag<?> binaryTag) {
        ...
        ((ListBinaryTag) compoundBinaryTag.getBinaryTag("properties")).getValue().forEach(property -> properties.add(binaryTagDriver.unMarshal(property, Property.class)));
